...
# Mujoco + OpenGL backend selection must happen *before* importing mujoco (important for Ray workers).
#
# On this machine, OSMesa + PyOpenGL import is failing with:
#   AttributeError: 'NoneType' object has no attribute 'glGetError'
# but EGL works (headless/offscreen via GPU drivers).
#
# Force EGL unconditionally so Ray worker envs can't inherit an incompatible
# backend (e.g. if the launcher set MUJOCO_GL=osmesa).
...
from utils.mujoco_utils import (
    get_canonical_pose,
    set_xml_light_params,
    find_non_collision_pose,
    save_robot_metadata,
)
